...
static TickType_t xTaskPeriods[schedMAX_NUMBER_OF_PERIODIC_TASKS] = {-1};
...
{
	// Serial.println("In Sched Periodic\n");
	// Serial.flush();
...
// Set dynamic priorities for EDF
...
static void prvCheckDeadline(SchedTCB_t *pxTCB, TickType_t xTickCount)
{
	...
	/* your implementation goes here */
	// Serial.println("Checking Deadline\n");
...
{
	SchedTCB_t *pxCurrentTask, pxTaskIter;
	TaskHandle_t xCurrentTaskHandle = xTaskGetCurrentTaskHandle();
	UBaseType_t flag, flag_edf = 0;
	BaseType_t xIndex;
	...
	for (xIndex = 0; xIndex < xTaskCounter; xIndex++)
	{
		if (((xCurrentTime - xSystemStartTime) % xTaskPeriods[xIndex]) == 0)
		{
			// Serial.println("Waking: ");
			// Serial.print(xTaskPeriods[xIndex]);
			// Serial.println(xCurrentTime - xSystemStartTime);
...
void vSchedulerStart(void)
{
	...
	SchedTCB_t *pxTaskIter;
	for (BaseType_t xIndex = 0; xIndex < schedMAX_NUMBER_OF_PERIODIC_TASKS; xIndex++)
	{

		pxTaskIter = &xTCBArray[xIndex];
		if (pxTaskIter->xInUse == pdTRUE)
		{
			xTaskPeriods[xIndex] = pxTaskIter->xPeriod;
		}
	}
